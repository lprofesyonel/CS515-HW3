...
# --- 4. STUDENT TRAINING (KNOWLEDGE DISTILLATION) ---
def train_student_kd(teacher: nn.Module, student: nn.Module, train_loader: DataLoader, device: torch.device, epochs: int = 10) -> nn.Module:
    """
    Trains a lightweight student model using Knowledge Distillation from a robust teacher.
    """
    print(f"\n--- Training Student (SimpleCNN) via KD from Robust Teacher for {epochs} Epochs ---")
    ...
    for epoch in range(epochs):
        ...
        for imgs, labels in train_loader:
            ...
            # Use standard KD loss (soft + hard loss)
            loss = kd_loss_fn(s_logits, t_logits, labels)
            loss.backward()
            optimizer.step()
            total_loss += loss.item()
            
        print(f"  Epoch {epoch+1}/{epochs} | KD Training Loss: {total_loss/len(train_loader):.4f}")
    return student

# --- 5. MAIN ORCHESTRATOR FOR HW3 ---
def run_robustness_evaluations(model: nn.Module, params: Dict[str, Any], device: torch.device):
    ...
    train_loader, clean_loader = get_loaders(params)
    ...
    for imgs, labels in clean_loader:
        ...
        # Teacher'ı kandırmak için üretilen adversarial resimler
        adv_imgs = pgd_linf(imgs, labels) 
